...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($company) || empty($email) || empty($contact) || empty($password)) {
        ...
        $check_sql = "SELECT * FROM recruiters WHERE email = ? OR contact = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO recruiters (name, company, email, contact, password) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $name, $company, $email, $contact, $password);
...
<body>
    <div class="container">
        ...
        <form action="R_reg.php" method="post">
