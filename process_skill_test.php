...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $correct_answers_query = "SELECT question_id, correct_option FROM questions WHERE skill_id = ?";
    ...
    $passed = $score >= 6; // Changed passing score to 6 for testing (adjust as needed)
    ...
    if ($passed) {
        ...
        if ($attempts >= 1) { // Only one attempt allowed (adjust as needed)
            ...
            $stmt_disqualify->bind_param("iii", $jobseeker_id, $job_id, $skill_id);
            $stmt_disqualify->execute();
            $stmt_disqualify->close();

            header("Location: search_jobs.php?test_failed=1&reason=disqualified");
            exit();
        } else {
            header("Location: skill_test.php?job_id=" . $job_id . "&test_failed=1&attempts_left=0"); // Redirect back to test with failure message
            exit();
        }
    }
} else {
